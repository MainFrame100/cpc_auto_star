...
*   **3.2.4. Хранение Данных:**
    ...
    *   Используются Unique Constraints для обеспечения уникальности записей (кампания/срез + неделя + аккаунт) и корректной работы UPSERT-логики при обновлении данных.
    *   Добавлена таблица DailyCampaignStat для хранения дневной статистики (сбор и использование запланированы на будущие вехи, не в MVP Вехи 5).
...
*   **3.3.4. Страница списка клиентов:** После авторизации, клиента перенаправляет на страницу со списком клиентов /auth/clients . На странице видны клиенты и список подключенных аккаунтов к каждому клиенту. Есть возможность добавить нового клиента, добавить аккаунт Яндекса к клиенту, обновить статистику по клиенту, перейти на страницу с просмотр статистики по клиенту (описана в пункте 3.3.1.)
