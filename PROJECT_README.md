...
*   **Подзадача 5.5: Единый Процесс Сбора Данных (2 этапа)**
    ...
    *   [ ] **Реализовать Шаг 1: Быстрое Обновление Списка Кампаний:**
        ...
        *   `ReportDefinition`: Период - **последняя неделя** (или 4 нед., если проще), Срезы - `CampaignId`, `CampaignName`, `CampaignType`, Метрики - **минимальные** (можно без них или базовые за последнюю неделю). **Без** фильтра `Impressions > 0`.
        *   Парсинг отчета.
        *   **`UPSERT`** в `WeeklyCampaignStat` **только для последней недели**, обновляя `CampaignName`, `CampaignType` и базовые метрики (если есть), **не затирая** детальные срезы. Использовать `ON CONFLICT DO UPDATE`.
